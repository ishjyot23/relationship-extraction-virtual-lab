...
def make_certificate_pdf(certificate_date):
    """Create a professional academic-style completion certificate.

    The layout is inspired by common university/virtual-lab certificate conventions.
    It is generated by this application and is not an official IIT Kharagpur certificate.
    """
    group_members = [
        "Shravani Khopade",
        "Soham Kesarkar",
    ...
    # Outer border
    cert.set_draw_color(55, 65, 81)
    cert.set_line_width(1.2)
    cert.rect(10, 10, 277, 190)
    cert.set_line_width(0.4)
    cert.rect(14, 14, 269, 182)

    # Header
    cert.set_text_color(40, 50, 65)
    cert.set_font("Helvetica", "B", 11)
    cert.cell(0, 8, "VIRTUAL LABORATORY", align="C", new_x="LMARGIN", new_y="NEXT")

    cert.set_font("Helvetica", "B", 24)
    cert.cell(0, 14, "CERTIFICATE OF COMPLETION", align="C", new_x="LMARGIN", new_y="NEXT")

    cert.set_font("Helvetica", "", 11)
    cert.cell(0, 8, "Academic Virtual Laboratory Experiment", align="C", new_x="LMARGIN", new_y="NEXT")
    ...
    cert.multi_cell(
        ...
        8,
        pdf_safe(
            "This is to certify that the following students have completed the "
            "virtual laboratory experiment on"
        ),
        align="C",
    )

    cert.set_font("Helvetica", "B", 17)
    cert.cell(0, 11, pdf_safe("Relationship Extraction from Text"), align="C", new_x="LMARGIN", new_y="NEXT")

    cert.ln(4)

    # Group members
    cert.set_font("Helvetica", "B", 11)
    cert.cell(0, 7, "Group Members", align="C", new_x="LMARGIN", new_y="NEXT")
    cert.set_font("Helvetica", "", 11)

    for member in group_members:
        cert.cell(0, 6, pdf_safe(member), align="C", new_x="LMARGIN", new_y="NEXT")
    ...
    cert.multi_cell(
        ...
        6,
        pdf_safe(
            "The experiment covered semantic relationship extraction, entity recognition, "
            "Subject–Relation–Object triples, graph visualization and self-evaluation."
        ),
        align="C",
    )
    ...
    cert.set_y(y)
    cert.set_x(left_x)
    cert.cell(70, 6, "________________________", align="C")
    cert.set_x(right_x)
    cert.cell(70, 6, "________________________", align="C")

    cert.set_y(y + 6)
    cert.set_x(left_x)
    cert.cell(70, 6, "Faculty / Lab Coordinator", align="C")
    cert.set_x(right_x)
    cert.cell(70, 6, "Date of Completion", align="C")

    cert.set_y(y + 12)
    cert.set_x(right_x)
    cert.cell(70, 6, pdf_safe(str(certificate_date)), align="C")
...
def certificate():
    ...
    st.write("Professional academic-style completion certificate for the virtual laboratory group.")
    ...
    st.info(
        "The certificate follows a professional academic/virtual-lab presentation style. "
        "It is generated by this application and is not an official IIT Kharagpur certificate."
    )

    if simulation_done and quiz_done:
        cert_pdf = make_certificate_pdf(certificate_date)
        st.success("Completion requirements satisfied. The certificate is ready to generate.")
        st.download_button(
        ...
        st.warning("Complete the Simulation and Quiz before generating the completion certificate.")
