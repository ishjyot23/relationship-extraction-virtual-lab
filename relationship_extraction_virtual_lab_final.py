...
def norm(x):return ' '.join(x.lower().split())

def relation(e1,e2,s):
    low=norm(s.text); a=min(e1.end_char,e2.end_char); b=max(e1.start_char,e2.start_char); between=norm(s.text[a:b])
    hits=[]
    for r,phrases in PATTERNS.items():
        for p in phrases:
            if p in between or p in low:hits.append((len(p),r))
    if hits:return sorted(hits,reverse=True)[0][1]
...
def extract(text):
    ...
    for e in doc.ents:
        ...
        if k not in seen:seen.add(k);ents.append({'Entity':e.text,'Type':e.label_})
    for s in doc.sents:
        es=list(s.ents)
        for i in range(len(es)):
            for j in range(i+1,len(es)):
                ...
                if k not in tseen:
                    tseen.add(k);triples.append({'Subject':sub.text,'Relation':r,'Object':obj.text,'Source Sentence':s.text.strip()})
    return ents,triples
...
def home():
    ...
    with a:
        ...
        for i,x in enumerate(['Identify named entities from unstructured text.','Extract semantic relationships between identified entities.','Represent relationships as Subject–Relation–Object triples.','Visualize the triples as a small directed semantic graph.'],1):st.write(f'**{i}.** {x}')
    with b:
...
def simulation():
    ...
    else:st.warning('No relation found. Try a clear sentence such as “Google developed Gemini.”')
    st.divider();st.header('Observation 3: Semantic Graph')
    if st.session_state.triples:st.plotly_chart(graph(st.session_state.triples),use_container_width=True);st.caption('Subject/Object = graph nodes; Relation = directed edge.')
    st.divider();st.header('Record Experimental Trial')
    if st.button('Record Current Trial'):
        st.session_state.trials.append({'Trial':len(st.session_state.trials)+1,'Entities':len(st.session_state.ents),'Triples':len(st.session_state.triples),'Input':st.session_state.text[:90],'Time':datetime.now().strftime('%H:%M:%S')});st.success('Trial recorded.')
...
def quiz():
    ...
    for i,(q,opts,ans) in enumerate(QUIZ,1):answers.append(st.radio(f'Q{i}. {q}',opts,key=f'q{i}'))
...
def result():
    ...
    if not st.session_state.done:st.warning('Perform the Simulation first.')
        ...
        if st.session_state.triples:st.dataframe(pd.DataFrame(st.session_state.triples)[['Subject','Relation','Object']],hide_index=True,use_container_width=True)
